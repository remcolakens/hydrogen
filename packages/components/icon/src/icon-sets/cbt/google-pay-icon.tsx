import { Icon } from '../../../src/icon';
import { IIconProps } from '../../../src/types';
const SvgGooglePayIcon = ({ className, ...props }: IIconProps) => {
	return (
		<Icon
			xmlnsXlink="http://www.w3.org/1999/xlink"
			viewBox="0 0 34 24"
			className={className}
			{...props}
		>
			<rect
				width={33}
				height={23}
				x={0.5}
				y={0.5}
				fill="#fff"
				stroke="currentColor"
				rx={3.5}
			/>
			<g clipPath="url(#google-pay-icon_svg__a)">
				<mask
					id="google-pay-icon_svg__c"
					width={8}
					height={10}
					x={17}
					y={7}
					maskUnits="userSpaceOnUse"
					style={{
						maskType: 'luminance',
					}}
				>
					<mask
						id="google-pay-icon_svg__b"
						width={8}
						height={10}
						x={17}
						y={7}
						maskUnits="userSpaceOnUse"
						style={{
							maskType: 'luminance',
						}}
					>
						<path
							fill="#fff"
							d="m17.958 13.83 3.682-6.526 2.006 1.184c1.295.765 1.74 2.46.99 3.785l-2.073 3.675a1.673 1.673 0 0 1-2.312.634l-1.862-1.1a1.227 1.227 0 0 1-.431-1.652Z"
						/>
					</mask>
					<g mask="url(#google-pay-icon_svg__b)">
						<path fill="#fff" d="M25 6.002H10V19h15V6.002Z" />
					</g>
				</mask>
				<g mask="url(#google-pay-icon_svg__c)">
					<mask
						id="google-pay-icon_svg__d"
						width={8}
						height={10}
						x={17}
						y={7}
						maskUnits="userSpaceOnUse"
						style={{
							maskType: 'luminance',
						}}
					>
						<path fill="#fff" d="M25 7.281h-7.21v9.548H25V7.28Z" />
					</mask>
					<g mask="url(#google-pay-icon_svg__d)">
						<path
							fill="url(#google-pay-icon_svg__e)"
							d="M0 0h7.674v9.756H0z"
							transform="matrix(1 0 0 -1 17.668 16.928)"
						/>
					</g>
				</g>
				<path
					fill="#FDBD00"
					d="m17.566 9.299-4.581 8.117 2.006 1.185c1.294.765 2.951.311 3.699-1.014l2.971-5.265a1.753 1.753 0 0 0-.62-2.366l-1.862-1.1a1.165 1.165 0 0 0-1.613.443Z"
				/>
				<path
					fill="#2DA94F"
					d="m21.64 7.302-1.42-.837c-1.618-.957-3.688-.39-4.623 1.268l-2.633 4.664c-.468.828-.19 1.888.62 2.366l1.42.838c.809.478 1.844.194 2.312-.635l3.143-5.57a2.334 2.334 0 0 1 3.23-.885L21.64 7.302Z"
				/>
				<mask
					id="google-pay-icon_svg__g"
					width={7}
					height={12}
					x={10}
					y={7}
					maskUnits="userSpaceOnUse"
					style={{
						maskType: 'luminance',
					}}
				>
					<mask
						id="google-pay-icon_svg__f"
						width={7}
						height={12}
						x={10}
						y={7}
						maskUnits="userSpaceOnUse"
						style={{
							maskType: 'luminance',
						}}
					>
						<path
							fill="#fff"
							d="m16.006 9.1-1.567-.924a1.444 1.444 0 0 0-1.993.545l-1.88 3.323c-.926 1.636-.377 3.728 1.226 4.672l1.193.703 1.446.852.627.37c-1.114-.764-1.468-2.293-.787-3.496l.487-.86 1.782-3.15a1.506 1.506 0 0 0-.534-2.036Z"
						/>
					</mask>
					<g mask="url(#google-pay-icon_svg__f)">
						<path fill="#fff" d="M25 6.002H10V19h15V6.002Z" />
					</g>
				</mask>
				<g mask="url(#google-pay-icon_svg__g)">
					<mask
						id="google-pay-icon_svg__h"
						width={7}
						height={12}
						x={10}
						y={7}
						maskUnits="userSpaceOnUse"
						style={{
							maskType: 'luminance',
						}}
					>
						<path fill="#fff" d="M16.745 7.965h-6.628v10.678h6.628V7.965Z" />
					</mask>
					<g mask="url(#google-pay-icon_svg__h)">
						<path
							fill="url(#google-pay-icon_svg__i)"
							d="M0 0h6.977v10.826H0z"
							transform="matrix(1 0 0 -1 10.038 18.708)"
						/>
					</g>
				</g>
			</g>
			<defs>
				<pattern
					id="google-pay-icon_svg__e"
					width={1}
					height={1}
					patternContentUnits="objectBoundingBox"
				>
					<use
						xlinkHref="#google-pay-icon_svg__j"
						transform="scale(.00752 .00606)"
					/>
				</pattern>
				<pattern
					id="google-pay-icon_svg__i"
					width={1}
					height={1}
					patternContentUnits="objectBoundingBox"
				>
					<use
						xlinkHref="#google-pay-icon_svg__k"
						transform="scale(.00826 .00546)"
					/>
				</pattern>
				<image
					xlinkHref="data:image/jpeg;base64,"
					id="google-pay-icon_svg__j"
					width={133}
					height={165}
				/>
				<image
					xlinkHref="data:image/jpeg;base64,"
					id="google-pay-icon_svg__k"
					width={121}
					height={183}
				/>
				<clipPath id="google-pay-icon_svg__a">
					<path fill="#fff" d="M10 6h15v13H10z" />
				</clipPath>
			</defs>
		</Icon>
	);
};
export default SvgGooglePayIcon;
